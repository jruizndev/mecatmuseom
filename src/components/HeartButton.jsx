...
import { getMemeById, updateMeme } from '../services/services'; // Importamos las funciones necesarias

const HeartButton = ({ initialLikes, memeId, onLike }) => {
  ...
  const toggleLike = async () => {
    ...
    try {
      ...
      // Llamamos a la función `onLike` que actualiza el estado en el componente padre
      if (onLike) {
        onLike(memeId, updatedLikes); // Pasamos el ID del meme y los likes actualizados
      }
    } catch (error) {
  ...
  return (
    <div className='flex items-center '>
      <button onClick={toggleLike} className='text-2xl bg'>
      ...
      <svg
          ...
          height='22'
          viewBox='0 0 24 22'
          fill={liked ? '#ff0000' : '#A3A3A3'} // Cambia el color dinámicamente
        >
          <path
